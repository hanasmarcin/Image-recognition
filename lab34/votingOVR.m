function votes = votingOVR(tset, clsmx)
...
%	clsmx(:,2) is "augmented dimension" coefficient (bias of sep. hyperplane)
%	clsmx(:,3:end) are regular separating hyperplane coefficients
% votes - output matrix of votes cast by all one-versus-one classifiers
	...
	for i=1:size(clsmx,1)
		...
		votes(res >= 0, pid) = votes(res >= 0, pid) + 1;
	end
